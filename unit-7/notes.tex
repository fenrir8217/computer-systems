...
\end{figure}
\begin{figure}
  \includegraphics[width=12cm]{unit-7/figures/device_controller.png}
...
\begin{itemize}
  \item System calls provide OS services to the user via an application programming interface (API).
  \item System calls APIs are typically written in C or C++, though some are written in assembly language.
  \item System calls are mostly accessed by programs via a high-level Application Programming Interface (API) rather than direct system call use
...
\begin{figure}
  \includegraphics[width=15cm]{unit-7/figures/05.png}
