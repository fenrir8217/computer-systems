...
The Network Layer provides the host-to-host communication service for the Internet.The key protocol on this layer is the Internet Protocol (IP).
\newline
The Network Layer provides a ‘Best Effort’ service.
\newline
There are therefore no guarantees: no speed, delay or bandwidth guarantees; nor any in-order delivery guarantee and not even any guarantee of delivery itself!
\newline
...
\newline
The network layer (router) performs forwarding and routing.
\begin{itemize}
  \item Forwarding is the movement of packets from the router input to the appropriate router output.A forwarding table determines local forwarding within a router.
    \begin{itemize}
      \item Like the process of planning a road trip from source to destination which includes many interchanges/junctions
    \end{itemize}
  \item Routing is the determination of the route taken by packets from their source to their destination.A routing algorithm determines the end-to-end path through the network.
    \begin{itemize}
      \item Like the process of going through an interchange/junction and choosing an appropriate exit lane
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Input Port – Forwarding
    \begin{itemize}
      \item Using the IP packet header’s destination IP address…
      \item …and the Forwarding Table in the input port’s memory…
      \item …lookup the appropriate router output port
    \end{itemize}
  \item Input Port - Queueing
    \begin{itemize}
      \item Goal: Complete input port processing as fast as packets arrives (at ‘line speed’)
      \item But if packets arrive faster than the forwarding rate, they will queue in the input port buffer.
    \end{itemize}
  \item Switching Fabrics
    \begin{itemize}
      \item Transfer packets from the input buffers to the appropriate output buffer
      \item Various different architectures, with different advantages and disadvantages
    \end{itemize}
  \item Switching Rate: The rate at which packets can be transferred from inputs to outputs
    \begin{itemize}
      \item Often measured in terms of the line rate
      \item For n inputs, a switching rate of n times the line rate is desirable.
    \end{itemize}
  \item Output Port – Queueing
    \begin{itemize}
      \item But if packets arrive faster than the transmission rate, they will queue in the output port buffer.
      \item A scheduling discipline chooses among the queued datagrams whom is next for transmission.
    \end{itemize}
  \item Output Port – Packet Loss
    \begin{itemize}
      \item As we have previously discussed, queueing/buffers lead to packet loss
      \item This can happen in both the output buffer and the input buffer (i.e. if the input buffer fills up because the switching fabric is slower than the input ports)
    \end{itemize}
\end{itemize}
...
When looking up a forwarding table entry for a given IP destination address, we use Longest Prefix Matching

\begin{figure}[htp]
  \centering
  \includegraphics[width=15cm]{unit-19/figures/01.png}
  \caption{Longest Prefix Matching}
\end{figure}
\subsection{\sout{Router Switching Fabrics}}
...
\subsection{\sout{Router Output Ports}}
...
An IP address is a \SI{32}{\bit} identifier for the host and router interface.More specifically, they identify each individual host or router interface
\newline
...
\newline
in the subnet part of the address
\begin{itemize}
  \item Remember: 32 bits in total!
  \item So, host part has 32 - x bits
\end{itemize}
\begin{tcolorbox}
    \color{blue}
    \Huge
    \uline{223.1.3}\textcolor{red}{.2} \\
    \large
    \textcolor{blue}{\uline{subnet part}} \hspace{0.5cm} \textcolor{red}{host part}
\end{tcolorbox}
...
\begin{itemize}
  \item As far as the outside world is concerned, all the hosts in a single local network use the same IP Address
    \begin{itemize}
      \item So only one IP Address is needed from the ISP for all devices on the network
      \item Local network devices can have their IP Addresses changed by the router without notifying the outside world
      \item And we can even change ISPs without changing the addresses of devices in the local network
    \end{itemize}
  \item Due to NAT, devices inside a local network are not explicitly addressable or even visible to the outside world – a security plus
\end{itemize} 
...
\begin{figure}
  \centering
  \includegraphics[width=15cm]{unit-19/figures/02.png}
  \includegraphics[width=15cm]{unit-19/figures/03.png}
  \caption*{NAT}
\end{figure}
...
% Ethernet Frame (outermost)
\draw[fill=red!80, draw=black, thick] (0,0) rectangle (14,4);
\node at (7,3.8) {\bfseries \textcolor{black}{Ethernet Frame}};

% IP Packet
\draw[fill=cyan!70, draw=black, thick] (1,0.5) rectangle (13,3.5);
\node at (7,3.3) {\bfseries \textcolor{black}{IP Packet}};

% TCP/UDP Segment
\draw[fill=green!70, draw=black, thick] (2,1.1) rectangle (12,3.1);
\node at (7,2.8) {\bfseries \textcolor{black}{TCP/UDP Segment}};

% Application Message (innermost)
\draw[fill=purple!60, draw=black, thick] (3,1.6) rectangle (11,2.6);
\node at (7,2.1) {\bfseries \textcolor{black}{Application Message}};
\end{tikzpicture}
\end{center}

\begin{figure}
  \centering
  \includegraphics[width=15cm]{unit-19/figures/04.png}
  \caption*{NAT}
\end{figure}