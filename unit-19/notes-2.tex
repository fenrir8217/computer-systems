...
\end{equation*}
\begin{flushright}
\(\functionsub{d}{v}{y}\): cost from neighbor v to destination y\\
\(\function{c}{x, v}\): cost to neighbor v\\
\(\functionsub{min}{v}{}\): min taken over all neighbors v of x\\
\end{flushright}
...
\subsection{Bellman-Ford – Example}
...
\begin{tikzpicture}[scale=1.5]
  ...
  \node[circle, draw, minimum size=8mm] (u) at (0,2) {u};
  \node[circle, draw, minimum size=8mm] (v) at (2,4) {v};
  \node[circle, draw, minimum size=8mm] (w) at (4,4) {w};
  \node[circle, draw, minimum size=8mm] (z) at (6,2) {z};
  \node[circle, draw, minimum size=8mm] (y) at (4,0) {y};
  \node[circle, draw, minimum size=8mm] (x) at (2,0) {x};
  ...
  \draw (w) -- (z);
  \draw (y) -- (z);
  \draw (u) to[bend left=75] (w);

  % Edge weights as raw text (not using node)
  \draw (1,3.1)  node[draw=none, fill=none, text=blue] {\small 2};
  \draw (1,1.1)  node[draw=none, fill=none, text=blue] {\small 1};
  \draw (3,4.2)  node[draw=none, fill=none, text=blue] {\small 3};
  \draw (2.3,2)  node[draw=none, fill=none, text=blue] {\small 2};
  \draw (3,-0.4) node[draw=none, fill=none, text=blue] {\small 1};
  \draw (3.1,2.3) node[draw=none, fill=none, text=blue] {\small 3};
  \draw (4.6,2)  node[draw=none, fill=none, text=blue] {\small 1};
  \draw (5,3.2)  node[draw=none, fill=none, text=blue] {\small 5};
  \draw (5,0.2)  node[draw=none, fill=none, text=blue] {\small 2};
  \draw (2,5)    node[draw=none, fill=none, text=blue] {\small 5};
\end{tikzpicture}
\end{center}

\begin{equation*}
  \functionsub{d}{x}{z} = \functionsub{min}{v}{\function{c}{x, v} + \functionsub{d}{v}{y}}
\end{equation*}
Clearly, \(\functionsub{d}{v}{z}\) = 5, \(\functionsub{d}{x}{z}\) = 3, \(\functionsub{d}{w}{z}\) = 3 \\
B-F equation gives: 
\begin{equation*}
\functionsub{d}{u}{z} = \function{min}{\function{c}{u,v} + \functionsub{d}{v}{z},\function{c}{u,x} + \functionsub{d}{x}{z}, \function{c}{u,w} + \functionsub{d}{w}{z}}
 = \function{min}{2 + 5, 1 + 3,5 + 3} = 4
\end{equation*}
