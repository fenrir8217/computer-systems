...
\begin{itemize}
  \item A system is parallel if it can perform more than one task simultaneously.
  \item A concurrent system supports more than one task by allowing each task to make progress.
  \item Concurrency can be achieved without parallelism.
\end{itemize}
...
\textcolor{blue}{Even a small amount of serial code has a noticeable impact on the overall performance.}
There are also overheads to parallelism that are not considered by Amdahl's law.
\textcolor{blue}{Latency} is introduced by \textcolor{blue}{memory access} and \textcolor{blue}{cache loading}, for example.
...
\subsection{\sout{Multithreading Models}}
\sout{
...
\subsubsection{\sout{Many-to-One Model}}
\sout{
...
\subsubsection{\sout{One-to-One Model}}
\sout{
...
\subsubsection{\sout{Many-to-Many Model}}
\sout{
...
\subsection{\sout{Thread Interference and Memory Consistency}}
\sout{
