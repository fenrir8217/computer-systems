...
A binary string is a string of any length that consists only of bits.
A 1-bit string is called a bit.
An 8-bit string is called a byte.
(Less common, but very cute: a 4-bit string is called a nibble.)

How many possible byte strings are
there?
Possible 1-bit strings = 2 = \(2 ^ 1\)
Possible 2-bit strings = 4 = \(2 ^ 2\)
Possible 3-bit strings = 8 = \(2 ^ 3\)
Possible 4-bit strings = 16 = \(2 ^ 4\)
Possible byte strings = \(2 ^ 8\) = 256

So, if we are limited to a single byte
...
\subsection{Comparison between difference methods}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Representation} & \textbf{Minimum} & \textbf{Maximum} \\
\hline
Usual & $0$ & $2^n - 1$ \\
\hline
...
\subsection{Representing Colour}

A mixture of primary colours!
In additive (RGB) colour models,every colour can be modelled as a mixture of red, green and blue, which is added to black.
In subtractive (CMY) colour models,every colour can be modelled as a mixture of cyan, magenta and yellow, which is subtracted from white.

So how should a given 24-bit binary word relate to a colour, which is a mixture of red, green
and blue?
Let’s split the 24 bits into three bytes.
